        ...
        case 'utility':
            embed.setTitle('🔧 Utility Commands')
                ...
                .addFields(
                    ...
                    { name: '₿ Crypto Tracker', value: `\`${prefix}crypto price <coin>\` - Check coin price\n\`${prefix}crypto <coin>\` - Quick price check\n\`${prefix}crypto top [n]\` - Top N coins by market cap\n\`${prefix}crypto info <coin>\` - Detailed coin info\n\`${prefix}crypto search <query>\` - Search coins\n\`${prefix}crypto watch <coin> #channel [%]\` - Set price alert (Admin)\n\`${prefix}crypto unwatch <coin>\` - Remove alert (Admin)\n\`${prefix}crypto alerts\` - View watchlist\n\`${prefix}crypto portfolio\` - View portfolio\n\`${prefix}crypto portfolio add <coin> <amt> [price]\` - Add holding\n\`/crypto price <coin>\` - Check price (slash)\n\`/crypto top [n]\` - Top coins (slash)\n\`/crypto info <coin>\` - Coin info (slash)\n\`/crypto portfolio\` - View portfolio (slash)\n\`/crypto portfolio-add <coin> <amt> [price]\` - Add holding (slash)\n\`/crypto watch <coin> #channel [%]\` - Set alert (slash)\n\`/crypto alerts\` - View alerts (slash)` },
                    { name: '🌤️ Weather', value: `\`${prefix}weather <city>\` - Get current weather\n\`${prefix}w <city>\` - Quick weather check\n\`/weather <city>\` - Get weather (slash)\n\n**Shows:** Temperature, humidity, wind, pressure, visibility, sunrise/sunset\n**API:** OpenWeatherMap (free, no key required for basic usage)` }
                );
            break;
